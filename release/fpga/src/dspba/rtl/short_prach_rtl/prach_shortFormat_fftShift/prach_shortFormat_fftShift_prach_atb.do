...
if {! [file exists $base_dir/prach_shortFormat_fftShift_prach]} {
    file delete -force $base_dir/prach_shortFormat_fftShift_prach
}

project new $base_dir prach_shortFormat_fftShift_prach
...
do "$base_dir/prach_shortFormat_fftShift/prach_shortFormat_fftShift_prach_fpc.do"
...
project addfile $base_dir/prach_shortFormat_fftShift/prach_shortFormat_fftShift_prach.sv systemverilog
...
puts {vcom -quiet -O0 $base_dir/prach_shortFormat_fftShift/prach_shortFormat_fftShift_prach_atb.vhd}
project addfile $base_dir/prach_shortFormat_fftShift/prach_shortFormat_fftShift_prach_atb.vhd vhdl
project addfile $base_dir/prach_shortFormat_fftShift/prach_shortFormat_fftShift_prach_stm.vhd vhdl
...
if {$vcomfailed == 0} {
    onbreak {
        ...
        if {$regOK == 1} {
            ...
            if { [catch {exa mismatch_fftShift_v_s} mismatch] == 0 && $mismatch } {
                report_mismatch fftShift_v_s $cyclenum
            }
            if { [catch {exa mismatch_fftShift_c_s} mismatch] == 0 && $mismatch } {
                report_mismatch fftShift_c_s $cyclenum
            }
            if { [catch {exa mismatch_fftShift_d_im} mismatch] == 0 && $mismatch } {
                report_mismatch fftShift_d_im $cyclenum
            }
            if { [catch {exa mismatch_fftShift_d_re} mismatch] == 0 && $mismatch } {
                report_mismatch fftShift_d_re $cyclenum
            }
    ...
    eval vsim -quiet -suppress 14408 -error 3473 -msgmode both -voptargs="+acc" -t ps prach_shortFormat_fftShift_prach_atb $ll
    do $base_dir/prach_shortFormat_fftShift/prach_shortFormat_fftShift_prach_atb.wav.do
