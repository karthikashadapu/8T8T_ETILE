...
-- VHDL created from standaloneTopLevel_ifft_blocktostream_DUT_low_phy_dl_iFFT_B2S_IFFT_VIFFT_BRandSCMap
...
entity standaloneTopLevel_ifft_blocktostream_DUT_low_phy_dl_iFFT_B2S_IFFT_VIFFT_BRandSCMap_atb is
end;

architecture normal of standaloneTopLevel_ifft_blocktostream_DUT_low_phy_dl_iFFT_B2S_IFFT_VIFFT_BRandSCMap_atb is

component standaloneTopLevel_ifft_blocktostream_DUT_low_phy_dl_iFFT_B2S_IFFT_VIFFT_BRandSCMap is
    port (
        ...
        x_im : in std_logic_vector(15 downto 0);  -- sfix16_en14
        x_re : in std_logic_vector(15 downto 0);  -- sfix16_en14
        FFTSize_s : in std_logic_vector(3 downto 0);  -- ufix4
        NSubCarr_s : in std_logic_vector(11 downto 0);  -- ufix12
        ...
        q_im : out std_logic_vector(15 downto 0);  -- sfix16_en13
        q_re : out std_logic_vector(15 downto 0);  -- sfix16_en13
        ready_s : out std_logic_vector(0 downto 0);  -- ufix1
        size_out_s : out std_logic_vector(3 downto 0);  -- ufix4
        cp_len_s : out std_logic_vector(10 downto 0);  -- ufix11
        q_rc_off_im : out std_logic_vector(15 downto 0);  -- sfix16_en14
        q_rc_off_re : out std_logic_vector(15 downto 0);  -- sfix16_en14
        fd_time_out_s : out std_logic_vector(63 downto 0);  -- ufix64
        clk : in std_logic;
        areset : in std_logic;
...
component standaloneTopLevel_ifft_blocktostream_DUT_low_phy_dl_iFFT_B2S_IFFT_VIFFT_BRandSCMap_stm is
    port (
        ...
        NSubCarr_s_stm : out std_logic_vector(11 downto 0);
        ...
        cp_len_s_stm : out std_logic_vector(10 downto 0);
        q_rc_off_im_stm : out std_logic_vector(15 downto 0);
        q_rc_off_re_stm : out std_logic_vector(15 downto 0);
        fd_time_out_s_stm : out std_logic_vector(63 downto 0);
        clk : out std_logic;
...
signal NSubCarr_s_stm : STD_LOGIC_VECTOR (11 downto 0);
...
signal cp_len_s_stm : STD_LOGIC_VECTOR (10 downto 0);
signal q_rc_off_im_stm : STD_LOGIC_VECTOR (15 downto 0);
signal q_rc_off_re_stm : STD_LOGIC_VECTOR (15 downto 0);
signal fd_time_out_s_stm : STD_LOGIC_VECTOR (63 downto 0);
...
signal NSubCarr_s_dut : STD_LOGIC_VECTOR (11 downto 0);
...
signal cp_len_s_dut : STD_LOGIC_VECTOR (10 downto 0);
signal q_rc_off_im_dut : STD_LOGIC_VECTOR (15 downto 0);
signal q_rc_off_re_dut : STD_LOGIC_VECTOR (15 downto 0);
signal fd_time_out_s_dut : STD_LOGIC_VECTOR (63 downto 0);
...
-- General Purpose data out check
checkready_s : process (clk, areset)
variable mismatch_ready_s : BOOLEAN := FALSE;
variable ok : BOOLEAN := TRUE;
begin
    IF ((areset = '1')) THEN
        -- do nothing during reset
    ELSIF (clk'EVENT AND clk = '0') THEN -- falling clock edge to avoid transitions
        ok := TRUE;
        mismatch_ready_s := FALSE;
        IF ( (ready_s_dut /= ready_s_stm) and qv_s_dut = "1") THEN
            mismatch_ready_s := TRUE;
            report "Mismatch on device output pin ready_s" severity Warning;
        END IF;
        IF (mismatch_ready_s = TRUE) THEN
            ok := FALSE;
        END IF;
        assert (ok)
        report "mismatch in device level signal." severity Failure;
...
begin
    ...
    ELSIF (clk'EVENT AND clk = '0') THEN -- falling clock edge to avoid transitions
        ...
        IF ( (size_out_s_dut /= size_out_s_stm) and qv_s_dut = "1") THEN
...
-- General Purpose data out check
checkcp_len_s : process (clk, areset)
variable mismatch_cp_len_s : BOOLEAN := FALSE;
variable ok : BOOLEAN := TRUE;
begin
    IF ((areset = '1')) THEN
        -- do nothing during reset
    ELSIF (clk'EVENT AND clk = '0') THEN -- falling clock edge to avoid transitions
        ok := TRUE;
        mismatch_cp_len_s := FALSE;
        IF ( (cp_len_s_dut /= cp_len_s_stm) and qv_s_dut = "1") THEN
            mismatch_cp_len_s := TRUE;
            report "Mismatch on device output pin cp_len_s" severity Warning;
        END IF;
        IF (mismatch_cp_len_s = TRUE) THEN
            ok := FALSE;
        END IF;
        assert (ok)
        report "mismatch in device level signal." severity Failure;
...
-- General Purpose data out check
checkq_rc_off_im : process (clk, areset)
variable mismatch_q_rc_off_im : BOOLEAN := FALSE;
variable ok : BOOLEAN := TRUE;
begin
    IF ((areset = '1')) THEN
        -- do nothing during reset
    ELSIF (clk'EVENT AND clk = '0') THEN -- falling clock edge to avoid transitions
        ok := TRUE;
        mismatch_q_rc_off_im := FALSE;
        IF ( (q_rc_off_im_dut /= q_rc_off_im_stm) and qv_s_dut = "1") THEN
            mismatch_q_rc_off_im := TRUE;
            report "Mismatch on device output pin q_rc_off_im" severity Warning;
        END IF;
        IF (mismatch_q_rc_off_im = TRUE) THEN
            ok := FALSE;
        END IF;
        assert (ok)
        report "mismatch in device level signal." severity Failure;
...
-- General Purpose data out check
checkq_rc_off_re : process (clk, areset)
variable mismatch_q_rc_off_re : BOOLEAN := FALSE;
variable ok : BOOLEAN := TRUE;
begin
    IF ((areset = '1')) THEN
        -- do nothing during reset
    ELSIF (clk'EVENT AND clk = '0') THEN -- falling clock edge to avoid transitions
        ok := TRUE;
        mismatch_q_rc_off_re := FALSE;
        IF ( (q_rc_off_re_dut /= q_rc_off_re_stm) and qv_s_dut = "1") THEN
            mismatch_q_rc_off_re := TRUE;
            report "Mismatch on device output pin q_rc_off_re" severity Warning;
        END IF;
        IF (mismatch_q_rc_off_re = TRUE) THEN
            ok := FALSE;
        END IF;
        assert (ok)
        report "mismatch in device level signal." severity Failure;
...
-- General Purpose data out check
checkfd_time_out_s : process (clk, areset)
variable mismatch_fd_time_out_s : BOOLEAN := FALSE;
variable ok : BOOLEAN := TRUE;
begin
    IF ((areset = '1')) THEN
        -- do nothing during reset
    ELSIF (clk'EVENT AND clk = '0') THEN -- falling clock edge to avoid transitions
        ok := TRUE;
        mismatch_fd_time_out_s := FALSE;
        IF ( (fd_time_out_s_dut /= fd_time_out_s_stm) and qv_s_dut = "1") THEN
            mismatch_fd_time_out_s := TRUE;
            report "Mismatch on device output pin fd_time_out_s" severity Warning;
        END IF;
        IF (mismatch_fd_time_out_s = TRUE) THEN
            ok := FALSE;
        END IF;
        assert (ok)
        report "mismatch in device level signal." severity Failure;
...
dut : standaloneTopLevel_ifft_blocktostream_DUT_low_phy_dl_iFFT_B2S_IFFT_VIFFT_BRandSCMap port map (
    busIn_writedata_stm,
    busIn_address_stm,
    busIn_write_stm,
    busIn_read_stm,
...
sim : standaloneTopLevel_ifft_blocktostream_DUT_low_phy_dl_iFFT_B2S_IFFT_VIFFT_BRandSCMap_stm port map (
    busIn_writedata_stm,
    busIn_address_stm,
    busIn_write_stm,
    busIn_read_stm,
