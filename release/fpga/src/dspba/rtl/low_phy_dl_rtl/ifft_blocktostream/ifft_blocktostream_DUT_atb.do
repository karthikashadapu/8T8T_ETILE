...
if {! [file exists $base_dir/ifft_blocktostream_DUT]} {
    file delete -force $base_dir/ifft_blocktostream_DUT
}

project new $base_dir ifft_blocktostream_DUT
...
do "$base_dir/ifft_blocktostream/ifft_blocktostream_DUT_fpc.do"
...
project addfile $base_dir/ifft_blocktostream/ifft_blocktostream_DUT.sv systemverilog
...
puts {vcom -quiet -O0 $base_dir/ifft_blocktostream/ifft_blocktostream_DUT_atb.vhd}
project addfile $base_dir/ifft_blocktostream/ifft_blocktostream_DUT_atb.vhd vhdl
project addfile $base_dir/ifft_blocktostream/ifft_blocktostream_DUT_stm.vhd vhdl
...
if {$vcomfailed == 0} {
    onbreak {
        ...
        if {$regOK == 1} {
            ...
            if { [catch {exa mismatch_td_ifft_out_data_im} mismatch] == 0 && $mismatch } {
                report_mismatch td_ifft_out_data_im $cyclenum
            }
            if { [catch {exa mismatch_td_ifft_out_data_re} mismatch] == 0 && $mismatch } {
                report_mismatch td_ifft_out_data_re $cyclenum
            }
            if { [catch {exa mismatch_td_ifft_out_ch_s} mismatch] == 0 && $mismatch } {
                report_mismatch td_ifft_out_ch_s $cyclenum
            }
            if { [catch {exa mismatch_td_ifft_out_valid_s} mismatch] == 0 && $mismatch } {
                report_mismatch td_ifft_out_valid_s $cyclenum
            }
    ...
    eval vsim -quiet -suppress 14408 -error 3473 -msgmode both -voptargs="+acc" -t ps ifft_blocktostream_DUT_atb $ll
    do $base_dir/ifft_blocktostream/ifft_blocktostream_DUT_atb.wav.do
