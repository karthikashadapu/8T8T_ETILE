...
%
% Author:      Nima Safari
% File:        prach_ZCseq_generate.m
% Description: Generate Zanoff-Chu sequence.

function ZCseq = prach_ZCseq_generate(ZCseq_num, circ_shift, N_zc)

  if N_zc == 839 % length of ZC sequence

      % root sequences from the first logical root selection set as in LTE PRACH format 0.
      % u(0), is used for first sequence, a moving uE is simulated by varying N_cs from 0 to Ncs.
      % cross correlation between seq1 and cyclically shofted seq1 (ie delay from an uE) and seq1 and seq2(unrelated root) are plotted.
  ...
  elseif N_zc == 139
      u = [1 138 2 137 3 136 4 135 5 134 6 133 7 132 8 131 9 130 10 129 ...
      ...
      error('The length of Zadoff Chu is wrong')
  end
  
  % ------------------------------------------------------------------------------ 
  % ZC_Seq Gen, Experimental spectrum synthesis in Freq Domain.
  % Generate ZC sequence and apply circular shift
  % (ZCseq_num+1 is to make the range 0-839 rather than 1-839
  %  this is needed as Matlab starts with u(1) not u(0))
  % ------------------------------------------------------------------------------ 
  n = 0:1:N_zc-1;
  ZCseq = exp(-1i*u(ZCseq_num+1)*(n.*(n+1))*pi/N_zc);
  
  ZCseq = circshift (ZCseq, circ_shift);
