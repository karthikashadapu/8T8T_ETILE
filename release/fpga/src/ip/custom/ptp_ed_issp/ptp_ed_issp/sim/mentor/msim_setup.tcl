...
# ----------------------------------------
# If ptp_ed_issp is one of several IP cores in your
...
# ----------------------------------------
# ACDS 24.1 115 linux 2025.10.16.13:57:28
...
if ![info exists TOP_LEVEL_NAME] { 
  set TOP_LEVEL_NAME "ptp_ed_issp.ptp_ed_issp"
}
...
set LD_LIBRARY_PATH [dict merge $LD_LIBRARY_PATH [dict get [ptp_ed_issp::get_env_variables $SIMULATOR_TOOL_BITNESS] "LD_LIBRARY_PATH"]]
if {[dict size $LD_LIBRARY_PATH] !=0 } {
  set LD_LIBRARY_PATH [subst [join [dict keys $LD_LIBRARY_PATH] ":"]]
  setenv LD_LIBRARY_PATH "$LD_LIBRARY_PATH"
}
append ELAB_OPTIONS [subst [ptp_ed_issp::get_elab_options $SIMULATOR_TOOL_BITNESS]]
append SIM_OPTIONS [subst [ptp_ed_issp::get_sim_options $SIMULATOR_TOOL_BITNESS]]
...
alias file_copy {
  ...
  set memory_files [concat $memory_files [ptp_ed_issp::get_memory_files "$QSYS_SIMDIR"]]
  ...
  set normalized_src [ptp_ed_issp::normalize_path "$file"]
  set normalized_dest [ptp_ed_issp::normalize_path "$dest_file"]
...
set libraries [dict merge $libraries [ptp_ed_issp::get_dpi_libraries "$QSYS_SIMDIR"]]
...
set design_libraries [dict merge $design_libraries [ptp_ed_issp::get_design_libraries]]
...
alias com {
  ...
  set design_files [dict merge [ptp_ed_issp::get_common_design_files $USER_DEFINED_COMPILE_OPTIONS $USER_DEFINED_VERILOG_COMPILE_OPTIONS $USER_DEFINED_VHDL_COMPILE_OPTIONS "$QSYS_SIMDIR"]]
  set common_design_files [dict values $design_files]
  foreach file $common_design_files {
    eval $file
  }
  set design_files [list]
  set design_files [concat $design_files [ptp_ed_issp::get_design_files $USER_DEFINED_COMPILE_OPTIONS $USER_DEFINED_VERILOG_COMPILE_OPTIONS $USER_DEFINED_VHDL_COMPILE_OPTIONS "$QSYS_SIMDIR"]]
