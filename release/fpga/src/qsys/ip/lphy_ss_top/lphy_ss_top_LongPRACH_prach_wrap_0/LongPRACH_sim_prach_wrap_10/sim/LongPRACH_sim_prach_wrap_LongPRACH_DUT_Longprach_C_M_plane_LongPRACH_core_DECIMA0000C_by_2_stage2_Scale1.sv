...
// SystemVerilog created from LongPRACH_sim_prach_wrap_LongPRACH_DUT_Longprach_C_M_plane_LongPRACH_core_DECIMATION1_Stage2_Filt_DEC_by_2_stage2_Scale1
...
module LongPRACH_sim_prach_wrap_LongPRACH_DUT_Longprach_C_M_plane_LongPRACH_core_DECIMA0000C_by_2_stage2_Scale1 (
    ...
    reg [21:0] redist0_u0_rnd_shift_q_1_q;
    ...
    wire [22:0] u0_rnd_add_a;
    wire [22:0] u0_rnd_add_b;
    logic [22:0] u0_rnd_add_o;
    wire [22:0] u0_rnd_add_q;
    ...
    assign u0_rnd_trunc_q = u0_rnd_trunc_qint[34:13];

    // u0_rnd_add(ADD,14)@0
    assign u0_rnd_add_a = $unsigned({{1{u0_rnd_trunc_q[21]}}, u0_rnd_trunc_q});
    assign u0_rnd_add_b = $unsigned({{21{u0_rnd_bias_q[1]}}, u0_rnd_bias_q});
    assign u0_rnd_add_o = $unsigned($signed(u0_rnd_add_a) + $signed(u0_rnd_add_b));
    assign u0_rnd_add_q = u0_rnd_add_o[22:0];
    ...
    // u0_high(COMPARE,9)@1
    assign u0_high_a = $unsigned({{8{u0_highconst_q[15]}}, u0_highconst_q});
    assign u0_high_b = $unsigned({{2{redist0_u0_rnd_shift_q_1_q[21]}}, redist0_u0_rnd_shift_q_1_q});
    assign u0_high_o = $unsigned($signed(u0_high_a) - $signed(u0_high_b));
    assign u0_high_c[0] = u0_high_o[23];

    // u0_lowconst(CONSTANT,6)
    assign u0_lowconst_q = $unsigned(16'b1000000000000001);

    // u0_low(COMPARE,8)@1
    assign u0_low_a = $unsigned({{2{redist0_u0_rnd_shift_q_1_q[21]}}, redist0_u0_rnd_shift_q_1_q});
    assign u0_low_b = $unsigned({{8{u0_lowconst_q[15]}}, u0_lowconst_q});
    assign u0_low_o = $unsigned($signed(u0_low_a) - $signed(u0_low_b));
    assign u0_low_c[0] = u0_low_o[23];
