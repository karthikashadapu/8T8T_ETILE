...
// SystemVerilog created from LongPRACH_sim_prach_wrap_LongPRACH_DUT_Longprach_C_M_plane_LongPRACH_core_S2B_FFT_Scale1
...
module LongPRACH_sim_prach_wrap_LongPRACH_DUT_Longprach_C_M_plane_LongPRACH_core_S2B_FFT_Scale1 (
    input wire [0:0] xIn_v,
    input wire [7:0] xIn_c,
    input wire [26:0] xIn_0,
    ...
    wire [22:0] u0_high_a;
    wire [22:0] u0_high_b;
    logic [22:0] u0_high_o;
    wire [0:0] u0_high_c;
    reg [15:0] u0_mux_q;
    reg [0:0] u0_mux_v;
    wire [26:0] u0_shiftsel_q;
    wire [26:0] u0_shiftsel_qint;
    wire [20:0] u0_rnd_trunc_q;
    wire [26:0] u0_rnd_trunc_qint;
    ...
    assign u0_shiftsel_q = u0_shiftsel_qint[26:0];

    // u0_rnd_trunc(BITSHIFT,13)@0
    assign u0_rnd_trunc_qint = u0_shiftsel_q;
    assign u0_rnd_trunc_q = u0_rnd_trunc_qint[26:6];
    ...
    // u0_high(COMPARE,9)@1
    assign u0_high_a = $unsigned({{7{u0_highconst_q[15]}}, u0_highconst_q});
    assign u0_high_b = $unsigned({{2{redist0_u0_rnd_shift_q_1_q[20]}}, redist0_u0_rnd_shift_q_1_q});
    assign u0_high_o = $unsigned($signed(u0_high_a) - $signed(u0_high_b));
    assign u0_high_c[0] = u0_high_o[22];

    // u0_lowconst(CONSTANT,6)
    assign u0_lowconst_q = $unsigned(16'b1000000000000001);

    // u0_low(COMPARE,8)@1
    assign u0_low_a = $unsigned({{2{redist0_u0_rnd_shift_q_1_q[20]}}, redist0_u0_rnd_shift_q_1_q});
    assign u0_low_b = $unsigned({{7{u0_lowconst_q[15]}}, u0_lowconst_q});
    assign u0_low_o = $unsigned($signed(u0_low_a) - $signed(u0_low_b));
    assign u0_low_c[0] = u0_low_o[22];
