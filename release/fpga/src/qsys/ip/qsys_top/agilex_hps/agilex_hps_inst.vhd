	component agilex_hps is
		port (
			...
			h2f_mpu_fpga_eventi     : in    std_logic                       := 'X';             -- fpga_eventi
			h2f_mpu_fpga_evento     : out   std_logic;                                          -- fpga_evento
			h2f_mpu_fpga_standbywfe : out   std_logic_vector(3 downto 0);                       -- fpga_standbywfe
			h2f_mpu_fpga_standbywfi : out   std_logic_vector(3 downto 0);                       -- fpga_standbywfi
			f2h_stm_hwevents        : in    std_logic_vector(43 downto 0)   := (others => 'X'); -- stm_hwevents
			h2f_cs_ntrst            : in    std_logic                       := 'X';             -- ntrst
			h2f_cs_tck              : in    std_logic                       := 'X';             -- tck
			h2f_cs_tdi              : in    std_logic                       := 'X';             -- tdi
			h2f_cs_tdo              : out   std_logic;                                          -- tdo
			h2f_cs_tdoen            : out   std_logic;                                          -- tdoen
			h2f_cs_tms              : in    std_logic                       := 'X';             -- tms
			...
			f2h_AWADDR              : in    std_logic_vector(36 downto 0)   := (others => 'X'); -- awaddr
			...
			f2h_ARADDR              : in    std_logic_vector(36 downto 0)   := (others => 'X'); -- araddr
			...
			f2h_AWDOMAIN            : in    std_logic_vector(1 downto 0)    := (others => 'X'); -- awdomain
			f2h_AWBAR               : in    std_logic_vector(1 downto 0)    := (others => 'X'); -- awbar
			f2h_ARDOMAIN            : in    std_logic_vector(1 downto 0)    := (others => 'X'); -- ardomain
			f2h_ARBAR               : in    std_logic_vector(1 downto 0)    := (others => 'X'); -- arbar
			f2h_ARSNOOP             : in    std_logic_vector(3 downto 0)    := (others => 'X'); -- arsnoop
			f2h_AWSNOOP             : in    std_logic_vector(2 downto 0)    := (others => 'X'); -- awsnoop
	...
	u0 : component agilex_hps
		port map (
			...
			h2f_mpu_fpga_eventi     => CONNECTED_TO_h2f_mpu_fpga_eventi,     --    h2f_mpu_events.fpga_eventi
			h2f_mpu_fpga_evento     => CONNECTED_TO_h2f_mpu_fpga_evento,     --                  .fpga_evento
			h2f_mpu_fpga_standbywfe => CONNECTED_TO_h2f_mpu_fpga_standbywfe, --                  .fpga_standbywfe
			h2f_mpu_fpga_standbywfi => CONNECTED_TO_h2f_mpu_fpga_standbywfi, --                  .fpga_standbywfi
			f2h_stm_hwevents        => CONNECTED_TO_f2h_stm_hwevents,        -- f2h_stm_hw_events.stm_hwevents
			h2f_cs_ntrst            => CONNECTED_TO_h2f_cs_ntrst,            --            h2f_cs.ntrst
			h2f_cs_tck              => CONNECTED_TO_h2f_cs_tck,              --                  .tck
			h2f_cs_tdi              => CONNECTED_TO_h2f_cs_tdi,              --                  .tdi
			h2f_cs_tdo              => CONNECTED_TO_h2f_cs_tdo,              --                  .tdo
			h2f_cs_tdoen            => CONNECTED_TO_h2f_cs_tdoen,            --                  .tdoen
			h2f_cs_tms              => CONNECTED_TO_h2f_cs_tms,              --                  .tms
			...
			f2h_AWDOMAIN            => CONNECTED_TO_f2h_AWDOMAIN,            --                  .awdomain
			f2h_AWBAR               => CONNECTED_TO_f2h_AWBAR,               --                  .awbar
			f2h_ARDOMAIN            => CONNECTED_TO_f2h_ARDOMAIN,            --                  .ardomain
			f2h_ARBAR               => CONNECTED_TO_f2h_ARBAR,               --                  .arbar
			f2h_ARSNOOP             => CONNECTED_TO_f2h_ARSNOOP,             --                  .arsnoop
			f2h_AWSNOOP             => CONNECTED_TO_f2h_AWSNOOP,             --                  .awsnoop
