...
# ---      alt_xcvr_native_helper_functions_sxa2k4i.tcl  --- #
# -                                                              - # 
# ---------------------------------------------------------------- #

set script_dir [file dirname [info script]] 
set split_qsys_output_name [split hssi_ss_1_xcvrnphy_fme_411_sxa2k4i "_"]
...
source "${script_dir}/${top_inst_name}_ip_parameters_sxa2k4i.tcl"
source "${script_dir}/alt_xcvr_native_helper_functions_sxa2k4i.tcl"

# Debug switch. Change to 1 in alt_xcvr_native_helper_functions_sxa2k4i.tcl to get more run-time debug information
...
# -                                                              - #
# ---------------------------------------------------------------- #
native_initialize_db_sxa2k4i nativedb_sxa2k4i

# ---------------------------------------------------------------- #
# --- Set all the instances of this core                       --- #
# ---------------------------------------------------------------- #
set alt_xcvr_native_s10_instances [ dict keys $nativedb_sxa2k4i ]
...
foreach inst $alt_xcvr_native_s10_instances {
  ...
  dict set alt_xcvr_native_s10_pins $inst [dict get $nativedb_sxa2k4i $inst]
  ...
  for {set i 0} {$i < $profile_cnt} {incr i} {
   ...
   # add for FM clocks
    ...
    dict for {mode mode_clks} $profile_clocks {
      ...
      set list_of_clk_names [native_prepare_to_create_clocks_all_ch_sxa2k4i $inst $num_channels $mode $mode_clks $profile_cnt $i $alt_xcvr_native_s10_pins $freq $multiply_factor_dict $divide_factor_dict $duty_cycle_dict $all_profile_clocks_names $tx_clkout_sel $tx_clkout2_sel $rx_clkout_sel $rx_clkout2_sel $protocol_mode]
    ...
    if {[get_collection_size $tx_coreclkin_fanin_col]} {
      ...
      foreach_in_collection fanin_node $tx_coreclkin_fanin_col {
        ...
        if {[dict exists $min_pulse_all_clocks_nodes_dict $fanin_node_name]} {
          ...
          add_ddr_report_command "native_check_special_min_pulse_sxa2k4i $fanin_clock_name"
        } 
      }
    }
  }
