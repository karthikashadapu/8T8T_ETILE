...
module hssi_ss_ip_top_p0_wctteii #(
// -----------------------------------------------------
...
//  Child IP parameters, port wise
    ...
    //
    parameter max_refclk          = 4,
    parameter max_clkout          = 6,
   ...
   output wire [1:0]		                    o_ptp_clk_pll,
   output wire [1:0]		                    o_ptp_clk_pll_div,
   output wire [1:0]		                    o_ptp_clk_rec_div64,
   output wire [1:0]		                    o_ptp_clk_rec_div,
   output wire [1:0]		                    o_ptp_xcvr_ch_pll_locked,
   //Reset Signals
  ...
  //XCVR Reconfig Interface, seperate one for each lane
   ...
   input wire [10-1:0]                      i_rsfec_reconfig_addr,
   input wire [8-1:0]                       i_rsfec_reconfig_writedata,
   output reg  [8-1:0]                      o_rsfec_reconfig_readdata,
   output reg                               o_rsfec_reconfig_readdatavalid,
   output wire                              o_rsfec_reconfig_waitrequest,

   input  wire [2-1:0]                      i_ptp_reconfig_write,
   input  wire [2-1:0]                      i_ptp_reconfig_read,
   input  wire [17*2-1:0]                   i_ptp_reconfig_address,
   input  wire [8*2-1:0]                    i_ptp_reconfig_writedata,
   output reg  [8*2-1:0]                    o_ptp_reconfig_readdata,
   output reg  [2-1:0]                      o_ptp_reconfig_readdatavalid,
...
//************************************************************************************************
    ...
    begin : XCVR_ADDR
      always @(posedge i_reconfig_clk)
      begin
        o_ptp_reconfig_readdatavalid[i]        <= i_ptp_reconfig_read[i] && ~o_ptp_reconfig_waitrequest[i];
        o_ptp_reconfig_readdata[(i*8)+7:(i*8)] <= o_ptp_reconfig_readdata_int[(i*8)+7:(i*8)];
      end

      assign o_ptp_reconfig_waitrequest[i] = (i_ptp_reconfig_read[i] | i_ptp_reconfig_write[i]) ? o_ptp_reconfig_waitrequest_int[i] : 1'b1;

      always_comb 
      begin
        if(i_ptp_reconfig_address[(i*17)+16] == 1'b1) //0x1_0000 - 0x1_FFFF
          i_ptp_reconfig_address_int[(i*19)+18:(i*19)] = {3'd0, i_ptp_reconfig_address[(i*17)+15:(i*17)]};
        else   //0x0_0000 - 0x0_FFFF
          i_ptp_reconfig_address_int[(i*19)+18:(i*19)] = {3'd4, i_ptp_reconfig_address[(i*17)+15:(i*17)]};
      end
    end
    endgenerate
    ...
    always @(posedge i_reconfig_clk) // or posedge i_reconfig_reset)
    begin
      if(i_reconfig_reset)
        rsfec_wait_cntr <= 11'd1250;
      else if(~o_rsfec_reconfig_waitrequest)
        rsfec_wait_cntr <= 11'd1250;
      else if(rsfec_wait_cntr > 11'd0)
        rsfec_wait_cntr <= rsfec_wait_cntr - 1'd1;
      else
        rsfec_wait_cntr <= 11'd0;
    end

    assign i_rsfec_reconfig_read_int = i_rsfec_reconfig_read && (rsfec_wait_cntr == 'd0);

    always @(posedge i_reconfig_clk)
    begin
      o_rsfec_reconfig_readdata      <= o_rsfec_reconfig_readdata_int;
      o_rsfec_reconfig_readdatavalid <= i_rsfec_reconfig_read_int && ~o_rsfec_reconfig_waitrequest;
    end
    assign i_rsfec_reconfig_addr_int = {1'b0, i_rsfec_reconfig_addr[9:0]};
    assign o_rsfec_reconfig_waitrequest = (i_rsfec_reconfig_read_int | i_rsfec_reconfig_write) ? o_rsfec_reconfig_waitrequest_int : 1'b1;

    generate
    for(i=0; i< NUM_CHANNELS; i=i+1)
    begin : TS_2D_CONV
      assign ss_app_st_txegrts0_tdata[(i*104)+95:(i*104)]     = o_ptp_ets[(i*96)+95:(i*96)];
      assign ss_app_st_txegrts0_tdata[(i*104)+103:(i*104)+96] = o_ptp_ets_fp[(i*8)+7:(i*8)];
    end
    endgenerate
    ...
    assign o_clk_pll[NUM_CHANNELS-1:0] = {NUM_CHANNELS{o_tx_pll_clk_div64[NUM_CHANNELS]}};

    `ifndef QUARTUS_CDC
    assign o_ptp_xcvr_ch_pll_locked[0]    = alt_ehipc3_fm_0.alt_ehipc3_fm_top_p0.alt_ehipc3_fm_hard_inst.sl_ptp_tx_pma_ready[0];
    assign o_ptp_xcvr_ch_pll_locked[1]    = alt_ehipc3_fm_0.alt_ehipc3_fm_top_p0.alt_ehipc3_fm_hard_inst.sl_ptp_tx_pma_ready[1];
    `else
    assign o_ptp_xcvr_ch_pll_locked[0]    = 1'b1; 
    assign o_ptp_xcvr_ch_pll_locked[1]    = 1'b1; 
    `endif

    assign o_ptp_clk_pll       = o_tx_pll_clk_div64[max_clkout-1:NUM_CHANNELS];
    assign o_ptp_clk_pll_div   = o_tx_pll_clk_div66[max_clkout-1:NUM_CHANNELS];
    assign o_ptp_clk_rec_div64 = o_rx_pll_clk_div64[max_clkout-1:NUM_CHANNELS];
    assign o_ptp_clk_rec_div   = o_rx_pll_clk_div66[max_clkout-1:NUM_CHANNELS];
...
//Core_Variant = 3 is only supported others are not, so core_variant=3 and act_ch=1 then 100G else 10/25G
      ...
      //
//ptp enabled in the same instance and aib enabled
	  	.i_aib_clk                        (o_ptp_clk_pll[0]),
	  	.i_aib_2x_clk                     (o_ptp_clk_pll_div[0]),
      //Reset signals
      ...
      // FEC AVMM
	  	.i_rsfec_reconfig_addr            (i_rsfec_reconfig_addr_int),       
	  	.i_rsfec_reconfig_read            (i_rsfec_reconfig_read_int),           
	  	.i_rsfec_reconfig_write           (i_rsfec_reconfig_write),          
	  	.o_rsfec_reconfig_readdata        (o_rsfec_reconfig_readdata_int),   
	  	.i_rsfec_reconfig_writedata       (i_rsfec_reconfig_writedata),      
	  	.o_rsfec_reconfig_waitrequest     (o_rsfec_reconfig_waitrequest_int),
      // PTP AVMM
	  	.i_ptp_reconfig_address           (i_ptp_reconfig_address_int),    
	  	.i_ptp_reconfig_read              (i_ptp_reconfig_read),           
	  	.i_ptp_reconfig_write             (i_ptp_reconfig_write),          
	  	.o_ptp_reconfig_readdata          (o_ptp_reconfig_readdata_int),   
	  	.i_ptp_reconfig_writedata         (i_ptp_reconfig_writedata),      
	  	.o_ptp_reconfig_waitrequest       (o_ptp_reconfig_waitrequest_int),
      // XCVR AVMM
      ...
      // PTP IF
	  	.i_ptp_tod                        (app_ss_st_txtod_tdata[95:0]), //TBD valid & width         //   input,   width = 96,                    ptp_tod_ports.i_ptp_tod
	  	.i_sl_ptp_ts_req                  (i_ptp_ts_req),              
	  	.i_sl_ptp_fp                      (i_ptp_fp),                  
	  	.o_sl_ptp_ets_valid               (ss_app_st_txegrts0_tvalid), 
	  	.o_sl_ptp_ets                     (o_ptp_ets),                 
	  	.o_sl_ptp_ets_fp                  (o_ptp_ets_fp),              
	  	.o_sl_ptp_rx_its                  (o_ptp_rx_its),  //TBD valid 
	  	.o_sl_tx_ptp_ready                (o_tx_ptp_ready),            
	  	.o_sl_rx_ptp_ready                (o_rx_ptp_ready),            
      //
	  	.i_sl_ptp_ins_ets                 (i_ptp_ins_ets),             
	  	.i_sl_ptp_ins_cf                  (i_ptp_ins_cf),              
	  	.i_sl_ptp_zero_csum               (i_ptp_zero_csum),           
	  	.i_sl_ptp_update_eb               (i_ptp_update_eb),           
	  	.i_sl_ptp_ts_format               (i_ptp_ts_format),           
	  	.i_sl_ptp_ts_offset               (i_ptp_ts_offset),           
	  	.i_sl_ptp_cf_offset               (i_ptp_cf_offset),           
	  	.i_sl_ptp_csum_offset             (i_ptp_csum_offset),         
	  	.i_sl_ptp_eb_offset               (i_ptp_eb_offset),           
	  	.i_sl_ptp_tx_its                  (i_ptp_tx_its),              
      //100G PTP
      //Custom IF
      //Misc Signals
